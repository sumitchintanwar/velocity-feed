...
func extractQuote(t *testing.T, ev marketdata.MarketEvent) marketdata.Quote {
	t.Helper()
	switch e := ev.(type) {
	case marketdata.Quote:
		return e
	case marketdata.PreEncodedEvent:
		var envelope struct {
			Type    string           `json:"type"`
			Payload marketdata.Quote `json:"payload"`
		}
		if err := json.Unmarshal(e.JSON, &envelope); err == nil && envelope.Payload.Symbol != "" {
			return envelope.Payload
		}
		var q marketdata.Quote
		if err := json.Unmarshal(e.JSON, &q); err == nil {
			return q
		}
		t.Fatalf("failed to parse Quote from PreEncodedEvent: %s", string(e.JSON))
	default:

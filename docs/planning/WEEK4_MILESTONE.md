...
**Current Phase:** End of Week 4 -- Reliability
**Status:** Core systems implemented, distributed with Redis Pub/Sub, reliability layer complete (event log, replay, snapshots, recovery, heartbeat, reconnect, message ordering)
...
6. [Week 4 -- Reliability](#6-week-4--reliability-days-22-28)
7. [Codebase Inventory](#7-codebase-inventory)
8. [Performance Benchmarks](#8-performance-benchmarks)
9. [Load Test Results](#9-load-test-results)
10. [Architecture Reviews](#10-architecture-reviews)
11. [Known Issues and Technical Debt](#11-known-issues-and-technical-debt)
12. [Week 5+ Roadmap](#12-week-5-roadmap)
13. [Multi-Gateway Deployment](#13-multi-gateway-deployment)
...
## 6. Week 4 -- Reliability (Days 22-28)

### Overview

Week 4 transformed the system from a real-time distribution pipeline into production-grade market data infrastructure. Every feature addresses a real failure scenario that trading systems face daily: client disconnects, gateway restarts, missed messages, and out-of-order delivery.

### End-of-Week 4 Deliverables

| Feature | Status | Tests | Design Doc |
|:---|:---:|:---:|:---|
| PostgreSQL Event Log | ✅ | 7 | Yes (1014 lines) |
| Replay API | ✅ | 18+ | Yes (1045 lines) |
| Snapshot Service | ✅ | 30+ | Yes (948 lines) |
| Restart Recovery | ✅ | 11 | Yes (1261 lines) |
| Heartbeat Detection | ✅ | 5+ | Yes (879 lines) |
| Automatic Reconnect | ✅ | 10+ | Yes (1094 lines) |
| Message Ordering | ✅ | 15+ | Yes (1071 lines) |
| Reliability Documentation | ✅ | N/A | 7 design + 6 review docs |

### Day 22-23: PostgreSQL Event Log

- `internal/eventlog/repository.go` (85 lines) -- `Repository` interface: `Append`, `AppendBatch`, `QueryBySymbol`, `QueryLatest`, `QueryEvents`, `Count`, `CountBySymbol`, `Close`
- `internal/eventlog/postgres.go` (504 lines) -- Full PostgreSQL implementation:
  - Partitioned `market_events` table (daily range partitioning on `timestamp`)
  - BRIN indexes for time-series queries, compound indexes for symbol+time lookups
  - Batch inserts with bulk `INSERT INTO ... VALUES (...)` pattern
  - Automatic partition management (creates today + 3 days ahead, runs every 12h)
  - Transient error retry logic (`isRetryable()`: connection reset, broken pipe, too many connections)
  - Connection pooling with configurable `MaxOpenConns`, `MaxIdleConns`, `MaxLifetime`
- `internal/eventlog/publisher.go` (215 lines) -- `PersistingPublisher` decorator: persists events to PostgreSQL before forwarding to inner publisher; batch buffering with configurable batch size and flush interval
- `internal/eventlog/migrations/001_create_market_events.sql` (23 lines) -- Idempotent schema migration
- `internal/eventlog/postgres_test.go` (442 lines) -- Tests: Append, AppendBatch, Count, QueryBySymbol, QueryLatest, RunMigrations, DSN generation
- Config: `DatabaseConfig` with Host, Port, User, Password, DBName, SSLMode, pool settings; env prefix `RTMDS_DATABASE_*`
- Dependency: `github.com/lib/pq v1.12.3`

### Day 23-24: Replay API

- `internal/transport/replay.go` (176 lines) -- `handleReplay` HTTP handler:
  - Query parameters: `symbol`, `from` (RFC3339), `to` (RFC3339), `cursor` (composite `timestamp,event_id`), `limit` (1-1000)
  - Per-client concurrency limiter (max 3 concurrent queries per IP, returns 429)
  - Client IP extraction from `X-Forwarded-For` / `X-Real-IP` headers
- `internal/transport/export.go` (168 lines) -- `handleReplayExport`: bulk export in CSV or NDJSON format, paginated streaming with max 100,000 events
- Routes: `GET /replay` and `GET /replay/export` (conditionally registered when `eventLog != nil`)
- `internal/transport/replay_test.go` (697 lines) -- 18+ tests: basic query, time range, cursor pagination, invalid parameters, empty query, repository error (500), concurrency limiting (429), export CSV/JSON, export pagination

### Day 24-25: Snapshot Service

- `internal/snapshot/snapshot.go` (717 lines) -- Complete in-memory snapshot cache:
  - `map[string]*snapshotEntry` with `sync.RWMutex`, O(1) lookups via `Get(symbol)` and `GetMultiple(symbols)`
  - Atomic pointer swaps for tear-free reads (immutable `*CachedEvent` values)
  - TTL-based eviction with configurable `maxAge` and background eviction loop
  - Dual-checkpoint persistence: current + `.prev` fallback, atomic writes via temp file + rename
  - `Recover()` method: `StartBuffering` -> `LoadCheckpoint` -> Replay missing events from event log -> `StopBuffering` (drains live events) -> `MarkReady`
  - Live-event buffering to fix the replay race condition (events arriving during DB replay captured in buffered channel)
  - `SnapshotPublisher` decorator: Feed -> SnapshotPublisher -> TopicManager
  - DB retry with exponential backoff during recovery
- `internal/snapshot/snapshot_test.go` (1299 lines) -- 30+ tests: Update/Get, concurrent access, TTL/eviction, atomic swap, checkpoint save/load/corruption, recovery with replay, restart scenario, dual rotation, health callback
- Config: `SnapshotConfig` with Enabled, CheckpointPath (`snapshot.json`), CheckpointInterval (30s), MaxAge (24h)
- New subscribers receive current market snapshots before live streaming begins (`internal/websocket/client.go:182-189`)

### Day 25-26: Restart Recovery

- `internal/recovery/recovery.go` (350 lines) -- `RecoveryManager` orchestrator:
  - Formal state machine: `Init` -> `Loading` -> `Connecting` -> `Recovering` -> `Building` -> `Ready` / `Failed`
  - Dependency registration, validation, and waiting with context cancellation
  - `Recover()` sequence: load config state -> wait for dependencies -> recover persistent state -> build in-memory state -> mark ready
  - State persistence to disk (JSON file with atomic writes)
  - Recovery counter tracking across restarts
  - `RecoveryReport` with state, duration, dependency status
  - Health callback on state transitions
- `internal/recovery/recovery_test.go` (292 lines) -- 11 tests: state transitions, dependencies, context cancellation, state persistence, full sequence
- Snapshot Service `Recover()` integration: dual-checkpoint fallback, live-event buffering, DB retry with backoff

### Day 26-27: Heartbeat Detection

- `internal/websocket/heartbeat.go` (187 lines) -- `HeartbeatManager`:
  - Centralized dead connection detection with per-client tracking (`heartbeatEntry` with `lastPong`, `pingSentAt`, `onTimeout` callback)
  - `Register()` / `Unregister()` / `RecordPing()` / `RecordPong()`
  - Background `Run()` loop scanning every 10s for dead connections
  - Server-initiated WebSocket ping every 30s, pong timeout at 90s (allows missing 3 pings)
  - Only triggers timeout when there is an outstanding ping (prevents false positives)
  - RTT metrics via `WSPingLatency` histogram
  - Prometheus metrics: `WSPingSentTotal`, `WSPongReceivedTotal`, `WSTimeoutsTotal`, `WSHeartbeatCleanupsTotal`
- Gateway integration (`internal/websocket/gateway.go`): creates `HeartbeatManager` on init, registers clients with `onTimeout` callback (disconnects dead clients), unregisters on disconnect
- Client integration (`internal/websocket/client.go`): sets `PongHandler` to record pings, write pump sends pings at 30s interval

### Day 27-28: Automatic Reconnect + Message Ordering

**Automatic Reconnect:**
- `internal/websocket/reconnect.go` (421 lines) -- `ReconnectClient`:
  - Exponential backoff: 1s initial, 30s max, 2x multiplier, +/-20% jitter
  - Subscription registry: client owns subscription state, automatically resubscribes on reconnect
  - `Subscribe()` / `Unsubscribe()` that send immediately when connected, or queue for next connection
  - `Start()` blocks until first connection, then runs reconnect loop in background
  - Dial timeout (5s) to prevent blocking on blackhole nodes
  - Prometheus metrics hooks: reconnect attempts, success, failures, resubscriptions
- `internal/websocket/reconnect_test.go` (447 lines) -- 10+ tests: backoff calculation, subscription registry, server restart integration, exponential backoff verification
- Public Go client SDK: `pkg/client/client.go` (370 lines) with reconnect support

**Message Ordering:**
- `internal/sequencer/sequencer.go` (120 lines) -- `Generator` interface + in-memory `Sequencer`:
  - Per-symbol monotonically increasing sequence numbers starting at 1
  - TTL-based eviction (5min default, configurable via `WithTTL()`)
  - Thread-safe via `sync.Mutex`
- `internal/sequencer/redis_sequencer.go` (101 lines) -- `RedisSequencer`:
  - Distributed `Generator` using Redis `INCR` for cross-instance consistency
  - Configurable key prefix (`seq:`), TTL support, `ResetSymbols()`
- `internal/sequencer/validator.go` (216 lines) -- `Validator`:
  - 16-shard concurrent validation with FNV-1a hash distribution
  - Detection: `Ok`, `Gap`, `OutOfOrder`, `Duplicate`
  - Per-symbol `Stats`: `TotalReceived`, `GapsDetected`, `OutOfOrderCount`, `Duplicates`, `LatestSeq`
  - TTL-based eviction, automatic eviction on each `Validate()` call
- Pipeline wiring: `NewPipeline()` accepts `Generator` interface; app selects Redis or in-memory based on `Redis.Enabled`
- Client-side: each WebSocket client holds a `Validator` for incoming message ordering validation

### Reliability Architecture

```
Client disconnects?
  → Reconnects automatically (exponential backoff + jitter)

Gateway restarts?
  → State recovers from checkpoint + event log replay

Client misses updates?
  → Replay API (cursor-based pagination, CSV/NDJSON export)

New subscriber joins?
  → Snapshot (latest prices) + live stream

Messages arrive out of order?
  → Sequence detection (16-shard validator, gap/out-of-order/duplicate)
```
...
```
internal/
  eventlog/         PostgreSQL event log (repository.go, postgres.go, publisher.go, migrations/)
  recovery/         Recovery manager with state machine (recovery.go)
  sequencer/        Sequence number generation + validation (sequencer.go, validator.go, redis_sequencer.go)
  snapshot/         In-memory snapshot service with checkpoint (snapshot.go)

pkg/
  client/           Public Go client SDK with reconnect (client.go)
```
...
### Go Packages (22 internal + 1 public)
...
internal/
  ...
  config/           Viper-backed configuration (config.go, persist.go)
  ...
  marketdata/       Domain types: Quote, Bar, MarketEvent, Feed, CachedEvent, Clock, Errors
  marketdata/simulator/  Simulated feed generator (simulator.go)
  ...
  recovery/         Recovery manager with state machine (recovery.go)
  sequencer/        Sequence generation + validation (sequencer.go, validator.go, redis_sequencer.go)
  snapshot/         In-memory snapshot service with checkpoint (snapshot.go)
  topicmanager/     Topic-based routing with COW fan-out (topicmanager.go, memory.go, router.go, groups.go)
  transport/        HTTP router + middleware (router.go, middleware.go, replay.go, export.go)
  websocket/        WebSocket gateway + client + heartbeat + reconnect (gateway.go, client.go, messages.go, heartbeat.go, reconnect.go)
  workerpool/       Bounded worker pool (pool.go, metrics.go)

pkg/
  client/           Public Go client SDK with reconnect support (client.go, 394 lines)

cmd/
  server/           Application entry point (main.go)
  benchmark/        Standalone distributed benchmark client (main.go)
  loadtest/         Standalone load test client (main.go)
  testclient/       Standalone WebSocket test client (main.go)
```

### Test Files (38 total)

- **Unit tests (20):** workerpool, clientqueue, feed, topicmanager (memory + router), websocket (gateway + reconnect), ratelimit, backpressure (channel + sim), pubsub, simulator, loadtest (2), eventlog, sequencer, recovery, snapshot, config (persist), client SDK
- **Benchmark tests (11):** workerpool, clientqueue, feed, topicmanager, websocket, transport, backpressure, pubsub, simulator, bench (e2e + backpressure)
- **Load test (2):** `internal/loadtest/loadtest_test.go`, `internal/loadtest/loadtest_scale_test.go`
...
- `cmd/loadtest/main.go` -- standalone load test client
- `cmd/testclient/main.go` -- standalone WebSocket test client
- `api/openapi.yaml` -- OpenAPI 3.1.0 specification (147 lines)
- `Makefile` -- build, test, lint, docker, multi-gateway targets
- `Dockerfile` -- multi-stage build (golang:1.23-alpine -> scratch)
- `Dockerfile.dev` -- development Dockerfile with Go + Redis
- `Dockerfile.benchmark` -- dedicated Dockerfile for benchmark client
- `docker-compose.yml` -- server, Redis, PostgreSQL, Prometheus, Grafana
...
- `internal/eventlog/migrations/001_create_market_events.sql` -- PostgreSQL schema migration
- `docs/diagrams/` -- Architecture diagrams (distributed_architecture.svg, multi_gateway_architecture.md)
- `docs/operations/` -- Operational runbooks (WSL_PROMETHEUS_VERIFICATION.md, WSL_TESTING.md)
- `docs/planning/ROADMAP.md` -- Full project roadmap (865 lines)

### Documentation (29 design + 32 review docs)
...
| BACKPRESSURE_STRATEGY.md | Backpressure design |
| BENCHMARKING_STRATEGY.md | Benchmarking approach |
| CHAOS_TESTING_DESIGN.md | Chaos testing framework |
| DISTRIBUTED_BENCHMARKING_STRATEGY.md | Distributed benchmark strategy |
| DISTRIBUTED_TOPIC_ROUTING_DESIGN.md | Distributed topic routing |
| FEED_GENERATOR.md | Feed generator design |
| HEARTBEAT_HANDLING_DESIGN.md | Heartbeat mechanism |
| LOAD_TESTING_STRATEGY.md | Load testing approach |
| MESSAGE_ORDERING_GUARANTEES_DESIGN.md | Sequence numbering & validation |
| MULTI_GATEWAY_ARCHITECTURE.md | Multi-gateway deployment |
| Observability_Design.md | Observability design (v1) |
| OBSERVABILITY_DESIGN_2.md | Observability design (v2) |
| PERSISTENT_EVENT_LOG_DESIGN.md | PostgreSQL event log |
| PER_SUBSCRIBER_BUFFERING_DESIGN.md | Per-subscriber buffer design |
| PUBLISHER_SERVICE.md | Publisher service design |
| RATE_LIMITING_DESIGN.md | Rate limiting design |
| REDIS_INTEGRATION_DESIGN.md | Redis Pub/Sub integration |
| REPLAY_API_DESIGN.md | Replay API design |
| RESTART_RECOVERY_DESIGN.md | Restart recovery design |
| SERVICE_DISCOVERY_DESIGN.md | Service discovery design |
| SNAPSHOT_SERVICE_DESIGN.md | Snapshot service design |
| STICKY_SESSION_DESIGN.md | Sticky session design |
| TOPIC_MANAGER.md | Topic manager design |
| WEBSOCKET_GATEWAY_DESIGN.md | WebSocket gateway design |
| WORKER_POOL_ARCHITECTURE.md | Worker pool architecture |
| WORKER_POOL_DESIGN.md | Worker pool design |
| AUTOMATIC_RECONNECT_DESIGN.md | Automatic reconnect design |

**Review Documents (`docs/reviews/`):**

| Document | Component |
|:---|:---|
| ARCHITECTURE_REVIEW.md | End-to-end architecture |
| BACKPRESSURE_REVIEW.md | Backpressure review |
| BENCHMARK_ANALYSIS.md | Benchmark analysis |
| chaos_testing_review.md | Chaos testing review |
| CODEBASE_ARCHITECTURE_REVIEW.md | Go implementation |
| distributed_benchmark_optimization.md | Distributed benchmark optimization |
| distributed_benchmark_review.md | Distributed benchmark review |
| distributed_routing_review.md | Distributed routing review |
| event_log_review.md | Event log review |
| FEED_REVIEW.md | Feed & distribution review |
| implementation_plan.md | Implementation plan |
| LOCKING_ARCHITECTURE_REVIEW.md | Locking architecture review |
| load_test_analysis.md | Load test analysis |
| multi_gateway_review.md | Multi-gateway review |
| observability_review.md | Observability review |
| OPTIMIZATION_REVIEW.md | sync.Map optimization review |
| ordering_review.md | Message ordering review |
| PUBLISHER_REVIEW.md | Publisher design review |
| rate_limiting_review.md | Rate limiter review |
| reconnect_review.md | Reconnect review |
| redis_integration_review.md | Redis integration review |
| restart_recovery_review.md | Restart recovery review |
| replay_api_review.md | Replay API review |
| service_discovery_review.md | Service discovery review |
| snapshot_service_review.md | Snapshot service review |
| sticky_session_review.md | Sticky session review |
| SUBSCRIBER_BUFFERING_REVIEW.md | Client queue review |
| TOPIC_MANAGER_ARCHITECTURE_REVIEW.md | Topic manager architecture review |
| TOPIC_MANAGER_REVIEW.md | Topic manager review |
| walkthrough.md | Code walkthrough |
| WEBSOCKET_GATEWAY_REVIEW.md | WebSocket gateway review |
| WORKER_POOL_REVIEW.md | Worker pool review |
...
| event_log_review | PostgreSQL Event Log | 0 | 0 | 2 | Implementation reviewed |
| replay_api_review | Replay API | 0 | 0 | 2 | Design reviewed |
| snapshot_service_review | Snapshot Service | 0 | 0 | 2 | Design reviewed |
| restart_recovery_review | Restart Recovery | 0 | 0 | 2 | Design reviewed |
| ordering_review | Message Ordering | 0 | 0 | 3 | Implementation reviewed |
...
### Week 4 -- Reliability (Days 22-28) ✅ COMPLETED

| Day | Task | Status |
|:---|:---|:---|
| 22 | PostgreSQL Event Log | ✅ Partitioned table, BRIN indexes, batch inserts, auto-partition management, retry logic |
| 23 | Replay API | ✅ Cursor-based pagination, CSV/NDJSON export, per-client concurrency limiter |
| 24 | Snapshot Service | ✅ Atomic pointer swaps, dual-checkpoint persistence, live-event buffering, TTL eviction |
| 25 | Recovery After Restart | ✅ State machine orchestrator, dependency management, checkpoint + event log replay |
| 26 | Heartbeat Detection | ✅ Centralized HeartbeatManager, 30s ping interval, 90s timeout, RTT metrics |
| 27 | Automatic Reconnect | ✅ Exponential backoff with jitter, subscription registry, dial timeout |
| 28 | Message Ordering | ✅ Redis INCR sequencer, 16-shard validator, gap/out-of-order/duplicate detection |
...
- `rtmds_websocket_ping_sent_total` (Counter)
- `rtmds_websocket_pong_received_total` (Counter)
- `rtmds_websocket_heartbeat_timeouts_total` (Counter)
- `rtmds_websocket_heartbeat_cleanups_total` (Counter)
- `rtmds_websocket_auth_failures_total` (Counter)
- `rtmds_websocket_handshake_duration_seconds` (Histogram)

### Reconnect Layer (4 metrics)
- `rtmds_websocket_reconnect_attempts_total` (Counter)
- `rtmds_websocket_reconnect_success_total` (Counter)
- `rtmds_websocket_reconnect_failures_total` (Counter)
- `rtmds_websocket_resubscriptions_total` (Counter)
...
### Rate Limiter (7 metrics)
- `rtmds_ratelimit_connect_allowed_total` (Counter)
- `rtmds_ratelimit_connect_rejected_total` (Counter)
- `rtmds_ratelimit_subscribe_allowed_total` (Counter)
- `rtmds_ratelimit_subscribe_rejected_total` (Counter)
- `rtmds_ratelimit_unsubscribe_allowed_total` (Counter)
- `rtmds_ratelimit_unsubscribe_rejected_total` (Counter)
- `rtmds_ratelimit_hits_total` (CounterVec: type)

### Distributed Routing (6 metrics)
- `rtmds_distributed_redis_subscriptions_active` (Gauge)
- `rtmds_distributed_redis_subscribe_total` (Counter)
- `rtmds_distributed_redis_unsubscribe_total` (Counter)
- `rtmds_distributed_events_routed_total` (Counter)
- `rtmds_distributed_symbols_with_local_subs` (Gauge)
- `rtmds_distributed_subscription_events_total` (CounterVec: action)

### Build Info (1 metric)
- `rtmds_build_info` (GaugeVec: version, revision)

**Total: 65 metrics** with bounded cardinality (no per-symbol, per-client, or per-IP labels).

---

## 13. Multi-Gateway Deployment

...
type SubscribeRequest struct {
	...
	ResumeSeq map[string]int64 `json:"resume_seq,omitempty"` // highest seq received per symbol
}

// ServerMessage is the envelope the server pushes to WebSocket clients.
type ServerMessage struct {
	Type    string `json:"type"`    // "quote" | "bar" | "error" | "subscribed" | "goaway"
	Payload any    `json:"payload"` // Quote | Bar | string | GoAwayMessage
}

// PingMessage is a heartbeat sent from Gateway to Client.
type PingMessage struct {
	Type      string `json:"type"`      // always "ping"
	Timestamp int64  `json:"timestamp"` // Unix epoch seconds
}

func (p PingMessage) EventSymbol() string { return "" }
func (p PingMessage) EventType() string   { return "ping" }

// PongMessage is a heartbeat reply from Client to Gateway.
type PongMessage struct {
	Type      string `json:"type"`      // always "pong"
	Timestamp int64  `json:"timestamp"` // Unix epoch seconds
}

func (p PongMessage) EventSymbol() string { return "" }
func (p PongMessage) EventType() string   { return "pong" }

// GoAwayMessage is a control message indicating the server is shutting down.
type GoAwayMessage struct {
	Reason string `json:"reason"`
}

func (g GoAwayMessage) EventSymbol() string { return "" }
func (g GoAwayMessage) EventType() string   { return "goaway" }
